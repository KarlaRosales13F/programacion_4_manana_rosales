enum class EstadoCanje(val descripcion: String, val esTerminal: Boolean) {
    ...
    fun puedeTransicionarA(siguiente: EstadoCanje): Boolean = when (this) {
...
fun main() {
    val estado = EstadoCanje.EN_PROCESO
    ...
    val icono = when (estado) {
        ...
        EstadoCanje.EN_PROCESO  -> "⏳"
        EstadoCanje.COMPLETADO  -> "✅"
        EstadoCanje.FALLIDO     -> "❌"
        EstadoCanje.CANCELADO   -> "🚫"
    }
    println(icono)  // ⏳

    println(estado.puedeTransicionarA(EstadoCanje.COMPLETADO))  // true
}