...
class PantallaDialogs extends StatelessWidget {
      ...
      SnackBar(
        content: Text(
          esError
              ...
              : 'Canje realizado correctamente en Loyaltee',
        ),
        backgroundColor:
  ...
  void _mostrarSnackBarAdvertencia(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('Advertencia: puntos próximos a expirar'),
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas eliminar a Ana Rosales?\n'
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Cliente "${ctrlNombre.text}" agregado',
          ),
        ),
      );
    }
  ...
  Future<void> _mostrarInformacion(BuildContext context) async {
    await showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'El programa Loyaltee está activo y el cliente no presenta alertas.',
        ),
        actions: [
          FilledButton(
            onPressed: () => Navigator.pop(ctx),
