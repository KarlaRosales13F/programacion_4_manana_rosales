    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.person, color: esCritico ? cs.error : cs.primary, size: 18),
              const SizedBox(width: 8),
              Text(metrica.servidor,
                  style: const TextStyle(fontWeight: FontWeight.bold)),
              const Spacer(),
              Text('${metrica.conexiones} canjes',
                  style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
            ]),
            const SizedBox(height: 10),
            _Barra('Puntos', metrica.cpu, cpuCritica),
            const SizedBox(height: 4),
            _Barra('Canjes', metrica.ram, ramCritica),
            const SizedBox(height: 4),
            _Barra('Retención', metrica.ssd, false),
          ],
        ),
      ),
    );
